...
Use the isolated
[`runtime/`](runtime/README.md) project to download and
...
```bash
uv run --project submission python experiments/E11-gopt-teacher/prepare.py \
...
docker run --rm --platform linux/amd64 \
  ...
  bash /workspace/experiments/E11-gopt-teacher/gopt_kaldi_extract.sh \
...
```bash
uv run --project submission python experiments/E11-gopt-teacher/attest.py batch-convert \
  --output-dir data/gopt_audits/kaldi-train-exact-converted

uv run --project submission python experiments/E11-gopt-teacher/attest.py batch-verify \
  --output-dir data/gopt_audits/kaldi-train-exact-converted
```
...
```bash
uv run --project experiments/E11-gopt-teacher/runtime gopt-score-batch \
...
uv run --project submission python experiments/E12-gopt-human-review/review.py sidecar-build \
  --data-dir data/dataset \
  --checkpoint data/gopt_models/official-gopt-librispeech/best_audio_model.pth \
...
uv run --project submission python experiments/E12-gopt-human-review/review.py sidecar-build \
  --data-dir data/dataset \
  --checkpoint experiments/E11-gopt-teacher/runtime/artifacts/best_audio_model.pth \
...
`accent_experiments.gopt_audit.write_jsonl_sidecar` is the only supported
producer.
...
```bash
uv run --project submission python experiments/E12-gopt-human-review/review.py review-prepare \
  --data-dir data/dataset \
...
```bash
uv run --project submission python experiments/E12-gopt-human-review/review.py review-status \
  --review-dir data/label_reviews/gopt-disagreements-seed42

uv run --project submission python experiments/E12-gopt-human-review/review.py review-reveal \
  --review-dir data/label_reviews/gopt-disagreements-seed42
```
