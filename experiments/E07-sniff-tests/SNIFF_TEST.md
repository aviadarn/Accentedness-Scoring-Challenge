...
Full phone-level reports belong under `runs/E07-sniff-tests/`. That directory
...
of the dataset with the documented `run.py` command.
...
uv run --project submission python experiments/E07-sniff-tests/run.py \
  ...
  --output runs/E08-own-voice/american.json

uv run --project submission python experiments/E07-sniff-tests/run.py \
  --audio data/sniff_test/non_native.wav \
  --phones "w i j ɝ b oʊ θ tʃ ɪ l d ɹ ʌ n t ʌ ɡ ɛ ð ɝ" \
  --output runs/E08-own-voice/non_native.json
```
