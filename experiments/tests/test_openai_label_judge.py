...
from accent_experiments.label_review import BlindReviewItem, load_review_packet
from accent_experiments.openai_label_judge import (
    APIJudgment,
    JudgeDecision,
...
def test_incomplete_run_keeps_private_key_sealed(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "accent_experiments.openai_label_judge._load_private_key", fail_if_unblinded
    )
    summary = run_audit(
        review_root,
        tmp_path / "output",
